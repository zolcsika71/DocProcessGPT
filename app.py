...
def delete_old_logs():
    ...
    if len(log_files) > 1:
        log_files.sort(
            key=lambda x: os.path.getctime(os.path.join(log_directory, x)), reverse=True
        )
...
def download_nltk_resources():
    ...
    for resource in resources:
        try:
            ...
            app.logger.info(f"NLTK resource '{resource}' is already available.")
        except LookupError:
            # Download the resource if not found
            try:
                nltk.download(resource, quiet=True)
                app.logger.info(f"NLTK resource '{resource}' downloaded successfully.")
            except Exception as e:
                app.logger.error(f"Error downloading NLTK resource '{resource}': {e}")
...
def view_logs():
    try:
        ...
        latest_log_file = max(
            log_files, key=lambda x: os.path.getctime(os.path.join(log_directory, x))
        )
